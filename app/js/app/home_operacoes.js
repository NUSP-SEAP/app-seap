...
`;
    ...
    async function loadMinhasOperacoes() {
        ...
        if (!resp || resp.ok === false) {
            ...
            tbody.innerHTML = `<tr><td colspan="7" class="empty-state">Erro ao carregar operações (HTTP ${status}). ${escapeHtml(msg)}</td></tr>`;
        ...
        if (data.length === 0) {
            tbody.innerHTML = `<tr><td colspan="7" class="empty-state">Nenhuma operação encontrada.</td></tr>`;
            renderPaginationControls("pag-minhas-operacoes", null, null, {
                report: { label: "Gerar Relatório", onClick: gerarRelatorioPdf }
            });
            return;
        ...
        data.forEach(op => {
            ...
            tr.innerHTML = `
                ...
                <td style="text-align:center">${fmtTime(op.pauta)}</td>
                <td style="text-align:center">${fmtTime(op.inicio)}</td>
                <td style="text-align:center">${fmtTime(op.fim)}</td>
